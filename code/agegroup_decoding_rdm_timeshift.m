function [decodingAccuracy_avgperm] = agegroup_decoding_rdm_timeshift()

n_perm=5; %originally 1000;
decodingAccuracy=NaN(n_perm, 190, 43);
...
for i_sub = 1:43
    ...
    load(eegfile);
    rdm(i_sub,:,:)=decodingAccuracy_matrixup(:,41:240);
...
for test_subject_index = 1:43
    ...
    for perm = 1:n_perm
        ...
        for time = 1:190
            ...
            train_data_1 = data(train_indices_1, :, time);
            train_data_2 = data(train_indices_2, :, time+4);
            train_data = [train_data_1;train_data_2];
            train_labels = labels(train_indices);
    
            if test_subject_index < 22
                test_data = squeeze(data(test_subject_index, :,time));
            else
                test_data = squeeze(data(test_subject_index, :,time+4));
            end
            ...
            decodingAccuracy(perm, counter, test_subject_index)=accuracy(1);
            counter = counter+1;
        end
    end
end
